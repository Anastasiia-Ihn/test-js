...
//   newItems.forEach(function (element) {totalPrice += element; });
...
//   const filteredNumbers = [];
//   // Change code below this line
// numbers.forEach(function(number) {if (number > value) {
//       filteredNumbers.push(number);
//     }})
...
// const totalAveragePlaytimePerGame = players.reduce((average, player) => { return average += (player.playtime/player.gamesPlayed)}, 0 );
// ;
...
// //   return users.reduce((previousValue, user) => { return previousValue += user.balance }, 0);
// // }
// //   ; console.log(calculateTotalBalance);

// // 37
// const getTotalFriendCount = users => {
//    return users.reduce((previousValue, user) => { return previousValue += user.friends.length }, 0);
...
//  return users.flatMap(user => user.friends).filter((friend, index, arr) => arr.indexOf(friend) === index).sort((a,b)=> a.localeCompare(b))
// };
...
// return users.filter( user => user.gender === gender).reduce((sum, element) => {return sum+= element.balance}, 0)
